...
class EditorGroupPanel(
  ...
  init {
    ...
    // this.fileEditor.putUserData<EditorGroupPanel2?>(EDITOR_PANEL, this)
  ...
  fun postConstruct() {
    ...
    val preferLatencyOverFlicker = state().isInitializeSynchronously
  ...
  private fun executeRefresh() {
    ...
    try {
      ...
      val displayedLinks = when {
        currentDisplayedGroup != null -> currentDisplayedGroup.getLinks(project) as MutableList<Link>
        else                          -> listOf<Link>()
      }
  ...
  internal inner class TabSelectionChangeHandler(val panel: EditorGroupPanel2) : KrTabs.SelectionChangeHandler {
    override fun execute(info: KrTabInfo?, requestFocus: Boolean, doChangeSelection: ActiveRunnable): ActionCallback {
      ...
      if (info is CustomGroupTabInfo) {
        panel._refresh(false, info.editorGroup)
      ...
      panel.openFile(
        editorGroupTabInfo.link, ctrl, shift, from(alt, shift)
      )
      return ActionCallback.DONE
    }
